...
- Coach portal Add Workout and Clients create flows use the collapsed full-width Add button pattern. Add Workout opens the workout builder; Add Client opens Create Client + Login, which creates the athlete profile and creates/links the Supabase Auth login in one step. Client edits happen inline inside each athlete card with its own Edit/Save controls. Client rows stay collapsed by default and include search to keep the page clean. Athlete Results are the single coach Progress page result view: tables grouped by movement, not stacked cards; the Progress Search/filter card sits at the top and filters those tables. Coach Saved Workouts show programs as the primary view: choose an athlete inside a program card to see that athlete’s logged reps, weights, scores, and notes in the same program layout. Coach Benchmarks manages leaderboard/test standards. Coach Movements manages non-benchmark exercises like Back Squat, Bench Press Barbell, Bench Press Dumbbell, and Deadlift Barbell for workouts/programs, with a Make Benchmark action when an exercise should move to benchmarks. On 2026-05-19, the live movement library was expanded with `supabase/standard-movements.sql`, adding 71 standard non-benchmark movements across strength, gymnastics, WOD, and cardio. Movement naming preference: generally use base movement first, then variation/equipment with spaces and no dash, e.g. `Assault Bike 10 Min Max Cal`, `Bench Press Barbell`, `Bench Press Dumbbell`, `Deadlift Barbell RDL`, `Deadlift Dumbbell RDL`, `Deadlift Hex Bar`, `Power Clean Barbell`, `Row Ring`, `Row 1K`, `Row 2K`, `Run 800m`, `Run 1 Mile`, and `Shoulder Push Press`; keep the conventional names `Back Squat` and `Front Squat`. Exact synonyms and reversed word order should share one display name so they sort together; do not merge distinct variations. Display Bodyweight WOD as `300 Rep Challenge`, categorize it as a timed WOD where lower time is better, require a time score instead of reps, and keep Angie separate. Categorize Hyrox and 24.1 entries as WODs. Cindy is a 20-minute rounds-plus-reps WOD where higher scores are better, and Cindy WOD aliases display as `Cindy`. Display `Side Lunge` as `Lateral Lunge` because it is the same movement, merge Incline Dumbbell Press under `Bench Press Incline Dumbbell`, merge Heavy Dumbbell Romanian Deadlift under `Deadlift Dumbbell RDL`, merge Dumbbell Squat Clean Technique under `Squat Clean Dumbbell`, standardize Kettle Bell Squatclean as `Squat Clean Kettlebell`, and label the 1, 2:30, 3, and 5 minute Assault Bike tests as `Max Cal`.
...
- Movement names with rep counts use natural numeric ordering, e.g. `Back Squat 5 Rep`, `Back Squat 10 Rep`, then `Back Squat 15 Rep`.
- The 24.1 Progress entry is named `24.1 Tester` and represents the actual tester only. Keep 24.1 primers, movement baselines, EMOMs, and interval sessions in their workout logs, but do not list them as separate Progress movements.
